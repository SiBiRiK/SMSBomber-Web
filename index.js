...
if (arguments.port) {
  app.listen(arguments.port, function () {
    console.log(`[SvPg | INFO] Server started on ${arguments.port}`)
    open(`http://localhost:${arguments.port}/`);
  });
} else {
  app.listen(3000, function () {
    console.log(`[SvPg | INFO] Server started on 3000`)
...
app.get('/attack', async (req, res) => {
  ...
  if (!Number(req.query.loops) || !req.query.loops) {
    ...
    return res.status(200).json({
      "success": true,
      "text": `[SVPG] Атака на номер +${req.query.number} успешно запущена`
    });
  }
});
...
app.get('/stop', async (req, res) => {
  if (req.query.number) {
    if (current_attacks[req.query.number]) {
      ...
      res.status(200).json({
        success: true,
        text: `[SVPG] Атака на номер +${req.query.number} успешно прекращена`
      });
    } else {
      return res.json({
        success: false,
        text: `[SVPG] Нет активных атак по номеру +${req.query.number}`
      });
    }
  } else {
    return res.json({
      success: false,
      text: `[SVPG] Параментр number не указал либо не является корректным`
    });
  }
});
...
app.get('/list', async (req, res) => {
  if (list_attacks.length === 0) {
    result = {
      success: false,
      text: `[SVPG] Список активных атак пуст`
    }
